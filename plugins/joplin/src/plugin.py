...
def check_installed(args: dict, request_id: str) -> dict:
    installed = shutil.which("joplin-desktop.exe") is not None or shutil.which("joplin.exe") is not None
    return {
...
def apply_config(args: dict, context: dict, request_id: str) -> dict:
    ...
    settings = args.get("settings") or args # Joplin issue example didn't explicitly wrap in settings but it's standard, handled by fallback
    
    # In the issue description input format:
    # "args": { "editor.codeView": true, ... } 
    # but the other plugins use "args": { "settings": { ... } }
    # So we should handle the actual object
    
    if "settings" in args:
        settings = args["settings"]
    elif "editor.codeView" in args or "theme" in args:
        settings = args
