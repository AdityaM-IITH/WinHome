...
def log(msg):
    sys.stderr.write(f"[lazydocker-plugin] {msg}\n")
    sys.stderr.flush()
...
def get_config_path():
    ...
    config_dir = os.path.join(appdata, "lazydocker")
...
def write_yaml(file_path: str, data) -> None:
...
def check_installed(args: dict, request_id: str) -> dict:
    installed = shutil.which("lazydocker.exe") is not None or shutil.which("lazydocker") is not None
    return {
...
def apply_config(args: dict, context: dict, request_id: str) -> dict:
    ...
    try:
        ...
        log(f"Updated lazydocker config: {config_path}")
