...
def dump_value(v):
    if isinstance(v, bool):
        ...
        items = [dump_value(item) for item in v]
        return "[" + ", ".join(items) + "]"
    else:
        return json.dumps(v, ensure_ascii=False)

def _dump_toml(data: dict, prefix: str = "") -> list:
    lines = []
    # Print primitives for the current table
    for k, v in data.items():
        if not isinstance(v, dict):
            # For keys with special chars, use json.dumps
            key_str = json.dumps(k, ensure_ascii=False) if not k.replace("_", "").replace("-", "").isalnum() else k
            lines.append(f"{key_str} = {dump_value(v)}")
            
    # Now print sub-tables
    for k, v in data.items():
        if isinstance(v, dict):
            ...
            lines.extend(_dump_toml(v, new_prefix))
    return lines
